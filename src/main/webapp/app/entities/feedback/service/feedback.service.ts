...
type RestOf<T extends IFeedback | NewFeedback> = Omit<T, 'feedbackDate'> & {
...
export class FeedbackService {
  ...
  protected convertDateFromClient<T extends IFeedback | NewFeedback | PartialUpdateFeedback>(feedback: T): RestOf<T> {
    return {
      ...feedback,
      feedbackDate: feedback.feedbackDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restFeedback: RestFeedback): IFeedback {
    return {
      ...restFeedback,
      feedbackDate: restFeedback.feedbackDate ? dayjs(restFeedback.feedbackDate) : undefined,
    };
  }
