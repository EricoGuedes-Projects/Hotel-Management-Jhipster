...
type RestOf<T extends IReservation | NewReservation> = Omit<T, 'checkInDate' | 'checkOutDate'> & {
...
export class ReservationService {
  ...
  protected convertDateFromClient<T extends IReservation | NewReservation | PartialUpdateReservation>(reservation: T): RestOf<T> {
    return {
      ...reservation,
      checkInDate: reservation.checkInDate?.toJSON() ?? null,
      checkOutDate: reservation.checkOutDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReservation: RestReservation): IReservation {
    return {
      ...restReservation,
      checkInDate: restReservation.checkInDate ? dayjs(restReservation.checkInDate) : undefined,
      checkOutDate: restReservation.checkOutDate ? dayjs(restReservation.checkOutDate) : undefined,
    };
  }
